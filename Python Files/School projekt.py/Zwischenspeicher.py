...
def Imagesearcher():
    Meldungen = 0
    def Total_Amount_ouf_Meldung_writing():
        Writing_Meldungen_Into_Document = open(r"C:\Users\User\Documents\Python Scripts\School projekt.py\Meldungen.txt","a")
        Writing_Meldungen_Into_Document.write("Insgesamte Meldungen während der ersten Stunde: "+ str(Meldungen) + "\n" + "\n")
        Writing_Meldungen_Into_Document.close()
    while True:
        if is_time_between(time(19,25),time(19,27)) or is_time_between(time(19,30),time(19,32)) or is_time_between(time(10,29),time(10,31)) or is_time_between(time(11,14),time(11,16)) or is_time_between(time(12,14),time(12,15)) or is_time_between(time(12,59),time(13,1)) ==True:
            Ende_der_Stunde = True
            print(Ende_der_Stunde)
            counter = 0
            while counter < 6:
                if counter == 0:
                    a = 10 # i gotta change these variables depending on which time it is
                    b = 55
                    c = 21
                    d = 40
                    ...
                    if is_time_between(time(a,b), time(c,d)) == True:  #while schleife?
                        ...
                        Total_Amount_ouf_Meldung_writing()
                        Meldungen = 0
                        t.sleep(60)
                        #f######################################################################
                        #########################################################################s#lepp#
